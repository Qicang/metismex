...
switch c
    case 'GLNXA64'
        path_to_metis = './Metis_build/build/';% you can modify the path here
        fprintf("The current path_to_metis is %s",path_to_metis);
        reply=input('\nDo you want to modify it [N/Y]?:','s');
        if isempty(reply)
            reply='Y';
        end

        if strcmp(upper(reply),'N')
